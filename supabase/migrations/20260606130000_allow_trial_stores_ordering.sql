-- Update create_public_order to check status in ('active', 'trial')
...
begin
  ...
  from public.stores
  where slug = p_store_slug and status in ('active', 'trial');
  ...
  insert into public.orders (
    id, store_id, order_number, customer_name, customer_phone, customer_email,
    customer_document, customer_id, delivery_type, delivery_date, notes,
    subtotal_cents, shipping_fee_cents, total_cents,
    shipping_region_id, shipping_region_name,
    delivery_zone_id, delivery_zone_name, delivery_distance_km,
    address_street, address_number, address_neighborhood, address_complement,
    address_city, address_state, national_shipping_cep,
    pix_name, status, payment_status,
    shipping_service_id, shipping_service_name, shipping_company, shipping_delivery_time_days
  ) values (
    gen_random_uuid(), v_store_id, v_order_number,
    trim(p_customer_name), p_customer_phone, p_customer_email,
    p_customer_document, p_customer_id, p_delivery_type, p_delivery_date, p_notes,
    0, v_shipping_fee_cents, 0,
    v_region_id, v_region_name,
    p_delivery_zone_id, coalesce(p_delivery_zone_name, v_final_zone_name), p_delivery_distance_km,
    p_address_street, p_address_number, p_address_neighborhood, p_address_complement,
    p_address_city, p_address_state, p_national_shipping_cep,
    p_pix_name, 'pending', 'unpaid',
    p_shipping_service_id, p_shipping_service_name, p_shipping_company, p_shipping_delivery_time_days
  )
  returning id into v_order_id;

  -- 6. Itens, Controle de Estoque e Variantes
  for v_item in select * from jsonb_array_elements(p_items)
  loop
    v_product_id         := (v_item->>'product_id')::uuid;
    v_quantity           := (v_item->>'quantity')::integer;
    v_variant_label      := v_item->>'variant_label';
    v_variant_option_ids := v_item->'variant_option_ids';
    ...
    -- Check whether this product has variant groups
    select exists(
      select 1 from public.product_variant_groups
      where product_id = v_product_id
    ) into v_has_variants;

    -- Decrement global stock only when product has NO variants
    if not v_has_variants and v_product.stock_qty is not null then
    ...
    insert into public.order_items (
      ...
      variant_label, variant_option_ids
    ) values (
      v_order_id, v_store_id, v_product.id, v_product.name,
      v_product.price_cents, v_quantity, v_line_total,
      v_variant_label,
      case
        when v_variant_option_ids is not null and jsonb_typeof(v_variant_option_ids) = 'array'
        then v_variant_option_ids
        else null
      end
    );
  end loop;
